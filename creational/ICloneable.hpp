...
private:
    ...
    static void destroy_impl(ICloneable* base, std::pmr::memory_resource* mr) noexcept {
        ...
        d->~Derived();
        mr->deallocate(d, sizeof(Derived), alignof(Derived));
    }
...
public:
    ...
    virtual std::unique_ptr<Derived, PmrDeleter> clone(std::pmr::memory_resource* mr = nullptr) const {
        using D = Derived;
        auto const& self = static_cast<D const&>(*this);

        if (!mr) {
            // Plain new/delete path
            return std::unique_ptr<Derived, PmrDeleter>(new D(self), PmrDeleter{}); // default-constructed deleter => delete
        }
        ...
        try {
            ...
            return std::unique_ptr<Derived, PmrDeleter>(p, PmrDeleter{ mr, &destroy_impl });
        } catch (...) {
